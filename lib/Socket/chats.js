...
const makeChatsSocket = (config) => {
    ...
    // Initialize message retry manager
    const messageRetryManager = config.messageRetryManager || (0, Utils_1.createMessageRetryManager)({ 
        maxRetries: config.maxMsgRetryCount || Defaults_1.DEFAULT_MAX_MSG_RETRY_COUNT,
        phoneRequestDelayMs: config.phoneRequestDelayMs || Defaults_1.DEFAULT_PHONE_REQUEST_DELAY_MS,
        logger 
    });
    if (!config.messageRetryManager) {
        config.messageRetryManager = messageRetryManager;
    }
