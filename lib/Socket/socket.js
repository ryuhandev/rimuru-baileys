...
const makeSocket = (config) => {
    ...
    const validateConnection = async () => {
        // Wait for pairing code if not registered yet
        if (!creds.registered && !creds.me) {
            logger.info('Waiting for pairing code registration...');
            // Don't proceed with handshake until pairing code is set
            return;
        }
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        ...
        const _defaultCode = 'RYHANDEV';
        // Validate and set pairing code
        ...
        else if (authState.creds.pairingCode && authState.creds.pairingCode.length === 8) {
            // Use existing code if already set and valid
            authState.creds.pairingCode = authState.creds.pairingCode.toUpperCase();
        }
        else {
            // Generate random code if no custom code provided
            authState.creds.pairingCode = _defaultCode;
        }
        // Set me credentials BEFORE emitting creds.update
        authState.creds.me = {
            id: (0, WABinary_1.jidEncode)(phoneNumber, 's.whatsapp.net'),
            name: '~'
        };
        // Emit creds update so listeners can react
    ...
    ws.on('open', async () => {
        try {
            // Only validate connection if we have creds.me (pairing code already requested)
            if (creds.me || creds.registered) {
                await validateConnection();
            } else {
                logger.info('WebSocket connected, waiting for pairing code...');
            }
        }
        catch (err) {
    ...
    // Listen for creds.update to trigger connection after pairing code is set
    ev.on('creds.update', async (update) => {
        // If pairing code was just set and we have me.id, trigger connection
        if (update.me?.id && ws.isOpen && !creds.registered) {
            logger.info('Pairing code set, initiating connection...');
            try {
                await validateConnection();
            }
            catch (err) {
                logger.error({ err }, 'error in validating connection after pairing');
            }
        }
    });
