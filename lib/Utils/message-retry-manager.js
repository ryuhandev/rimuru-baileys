...
exports.createMessageRetryManager = createMessageRetryManager;
...
function createMessageRetryManager(options) {
    const { logger, maxRetries, phoneRequestDelayMs } = options;
    return new MessageRetryManager(logger, maxRetries, phoneRequestDelayMs);
}

class MessageRetryManager {
    constructor(logger, maxMsgRetryCount, phoneRequestDelayMs = PHONE_REQUEST_DELAY) {
        this.logger = logger;
        this.maxMsgRetryCount = maxMsgRetryCount;
        this.phoneRequestDelayMs = phoneRequestDelayMs;
    ...
    shouldRecreateSession(jid, hasSession) {
        ...
        const retryCount = this._retryCounters.get(jid) || 0;
        if (retryCount < 2) {
